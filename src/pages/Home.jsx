            ...
            {user?.role === 'admin' && (
              <div className={`mt-6 rounded-2xl border p-4 ${isDark ? 'border-emerald-900 bg-emerald-950' : 'border-emerald-200 bg-emerald-50'}`}>
                ...
                <div className="mt-3 flex flex-wrap gap-3">
                  ...
                  <Link
                    to="/admin/events"
                    className={`inline-flex items-center rounded-full border px-5 py-3 text-sm font-semibold transition ${isDark ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-slate-200 text-slate-900 hover:bg-slate-100'}`}
                  >
                    Create event
                  </Link>
                </div>
                <p className={`mt-3 text-xs ${isDark ? 'text-emerald-400/60' : 'text-emerald-700/80'}`}>
          ...
          {filteredEvents.map((event) => (
            <article key={event.id} className={`overflow-hidden rounded-3xl border shadow-lg transition hover:-translate-y-1 ${isDark ? 'border-slate-700 bg-slate-800 hover:border-sky-400' : 'border-slate-200 bg-white hover:border-sky-300'}`}>
              ...
              <div className="space-y-4 p-5">
                ...
                <Link to={`/events/${event.id}`} className="inline-flex rounded-full bg-sky-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-400">
                  View details
                </Link>
              </div>
            </article>
